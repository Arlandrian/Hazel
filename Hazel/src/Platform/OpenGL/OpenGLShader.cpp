...
namespace Hazel {
	...
	void OpenGLShader::SetInt(const std::string& name, const unsigned int value) const
	{
	...
	std::string OpenGLShader::ReadFile(const std::string& srcPath) const {
		std::string result;
		...
		if( in ) {
			...
			HZ_CORE_ERROR("Could not open file {0}.", srcPath);
		}
		return result;
	}
